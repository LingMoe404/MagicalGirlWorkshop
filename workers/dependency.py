...
from utils import get_config_path, get_subprocess_flags, safe_decode, tool_path
                        ...
                        if proc.returncode == 0:
                            ...
                            if err_msg:
                                self.log_signal.emit(
                                    tr(
                                        "log.dependency.qsv_failed",
                                        error=err_msg.splitlines()[0],
                                    ),
                                    "error",
                                )
                ...
                try:
                    with subprocess.Popen(
                        ...
                        if proc.returncode == 0:
                            ...
                            target_msg = (
                                err_msg.split("->")[-1] if "->" in err_msg else err_msg
                            )
                            short_err = (
                                target_msg.strip().splitlines()[0]
                                if target_msg and target_msg.strip()
                                else tr("common.unknown_error")
                            )

                            # 无论什么原因导致失败，都输出一条错误日志
                            self.log_signal.emit(
                                tr("log.dependency.nvenc_failed", error=short_err),
                                "error",
                            )
