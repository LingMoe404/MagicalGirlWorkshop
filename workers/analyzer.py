...
class AnalysisWorker(BaseWorker):
    ...
    def run(self):
        ...
        try:
            ...
            html.append(
                f'<div style="color: {key_color}; font-size: 11px; margin-top: 4px;">{self.filepath}</div>'
            )
            html.append("</div>")
            ...
            html.append(
                f'<span style="color: {key_color};">{tr("info.report.format")}</span> <span style="color: {val_color};">{fmt.get("format_long_name", "Unknown")}</span><br/>'
            )
            html.append(
            ...
            for stream in data.get("streams", []):
                ...
                if st_type == "VIDEO":
                    ...
                    html.append(
                        f'<span style="color: {key_color};">{tr("info.report.codec")}</span> <span style="color: {val_color};">{codec_display}</span><br/>'
                    )
                    html.append(
                        f'<span style="color: {key_color};">{tr("info.report.profile_level")}</span> <span style="color: {val_color};">{stream.get("profile", "N/A")} @ Level {stream.get("level", "N/A")}</span><br/>'
                    )
                    html.append(
                        f'<span style="color: {key_color};">{tr("info.report.resolution")}</span> <span style="color: {val_color};">{stream.get("width")} x {stream.get("height")} (DAR: {stream.get("display_aspect_ratio", "N/A")})</span><br/>'
                    )
                    ...
                    if not bit_depth or str(bit_depth) == "0":
                        if "16le" in pix_fmt or "16be" in pix_fmt:
                            ...
                            "10le" in pix_fmt or "10be" in pix_fmt or "p010" in pix_fmt
                        ):
                            bit_depth = "10"
                    ...
                    html.append(
                        f'<span style="color: {key_color};">{tr("info.report.pix_fmt")}</span> <span style="color: {val_color};">{pix_fmt} ({bit_depth} bit)</span><br/>'
                    )
                    html.append(
                        f'<span style="color: {key_color};">{tr("info.report.color_space")}</span> <span style="color: {val_color};">{stream.get("color_space", "N/A")} / {stream.get("color_range", "N/A")}</span><br/>'
                    )
                    ...
                    html.append(
                        f'<span style="color: {key_color};">{tr("info.report.sample_rate")}</span> <span style="color: {val_color};">{stream.get("sample_rate")} Hz</span><br/>'
                    )
                    html.append(
                        f'<span style="color: {key_color};">{tr("info.report.sample_fmt")}</span> <span style="color: {val_color};">{stream.get("sample_fmt", "N/A")}</span><br/>'
                    )
                    html.append(
                        f'<span style="color: {key_color};">{tr("info.report.channel_layout")}</span> <span style="color: {val_color};">{stream.get("channels")} ch ({stream.get("channel_layout", "N/A")})</span><br/>'
                    )
                    if "bit_rate" in stream:
                        html.append(
                            f'<span style="color: {key_color};">{tr("info.report.bitrate")}</span> <span style="color: {val_color};">{int(stream.get("bit_rate")) / 1000:.0f} kbps</span><br/>'
                    ...
                    if "tags" in stream and "language" in stream["tags"]:
                        html.append(
                            f'<span style="color: {key_color};">{tr("info.report.language")}</span> <span style="color: {val_color};">{stream["tags"]["language"]}</span><br/>'
                        )
                    html.append("</div>")
