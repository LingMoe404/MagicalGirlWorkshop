...
class FakeProcess:
    def __init__(self, stdout_data, returncode=0, stderr_data=b""):
        self._stdout = (
            ...
            else stdout_data.encode("utf-8")
        )
        self._stderr = (
            stderr_data
            if isinstance(stderr_data, bytes)
            else stderr_data.encode("utf-8")
        )
...
class AnalysisWorkerTests(unittest.TestCase):
    def _run_analysis(self, ffprobe_data, filepath="test.mp4", is_dark=False):
        ...
        with (
            ...
            patch("qfluentwidgets.isDarkTheme", return_value=is_dark),
            patch("workers.analyzer.tool_path", return_value="ffprobe.exe"),
        ):
    ...
    def test_mkv_av1_shows_perfect_form_badge(self):
        worker = self._run_analysis(
            _make_ffprobe_output(is_mkv=True, codec="av1"), filepath="test.mkv"
        )
    ...
    def test_mp4_h264_does_not_show_perfect_form(self):
        worker = self._run_analysis(
            _make_ffprobe_output(is_mkv=False, codec="h264"), filepath="test.mp4"
        )
    ...
    def test_ffprobe_failure_emits_error_html(self):
        ...
        with (
            patch("subprocess.Popen") as mock_popen,
            patch("qfluentwidgets.isDarkTheme", return_value=False),
            patch("workers.analyzer.tool_path", return_value="ffprobe.exe"),
        ):
...
class ThumbnailWorkerTests(unittest.TestCase):
    ...
    def test_thumbnail_generated(self, mock_from_data):
        with (
            ...
            patch("subprocess.Popen") as mock_popen,
            patch("workers.analyzer.tool_path", return_value="ffmpeg.exe"),
        ):
