...
- [x] 从 `config.py` 读取版本并写入 `GITHUB_ENV`。
- [x] 将 Nuitka 参数从 `--onefile` 改为 `--standalone`。
- [x] 将 `main.dist` 和 `tools` 合并到统一暂存目录。
- [x] 生成带版本号的 Portable 7z。
- [x] 确保 Inno Setup 可用并编译 Setup.exe。
- [x] 显示两种产物大小并分别上传 artifact。

### Task 3: 验证

- [x] 使用 PyYAML 解析工作流文件。
- [x] 搜索确认构建命令不再包含 `--onefile`。
- [x] 静态检查 Inno Setup 的用户级目录、源目录、快捷方式与卸载规则。
- [x] 运行完整 Python 测试集。
- [x] 检查最终 diff 和 Git 工作区。
