...
- [x] 将主界面页脚改为作者署名和 AI 协作短版。
- [x] 将致谢页脚统一为 Codex、GPT、Antigravity 与 Gemini。
- [x] 运行 `uv run python check_lang.py`，预期四种语言键完全一致。
...
- [x] 将 Gemini 单独徽章改为多模型 AI 协作徽章。
- [x] 更新简介中的 Powered by 文本。
- [x] 在致谢和开发幕后中说明作者主导及多 AI 协作，删除“100% Gemini 生成”。

### Task 3: 验证

- [x] 搜索当前项目中的旧 Gemini 独占描述，确认仅历史发布记录保留。
- [x] 运行语言一致性检查。
- [x] 离屏初始化主窗口，确认页脚文本包含四个协作名称。
